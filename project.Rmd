...
```{r message=FALSE, warning=FALSE}
data <- data[, c('Sex', 'Salary', 'RaceDesc', 'Department', 'RecruitmentSource', 'DateofHire')] # select only the columns that we need
```

```{r message=FALSE, warning=FALSE}
sum(is.na(data)) # sum of rows that contain NA values
```
```{r message=FALSE, warning=FALSE}
# na.omit(data)    # remove all rows that contain NA values
...
```{r message=FALSE, warning=FALSE}
# Convert the date in chr type to date format
data <- data %>%
...
```{r}
data$Sex <- reorder(data$Sex, data$Sex, function(x)-length(x)) # Order the Sex data by its total number

data$Department <- reorder(data$Department, data$Department, function(x)-length(x)) # Order the Department data by the number of workers in it

data$RaceDesc <- reorder(data$RaceDesc, data$RaceDesc, function(x)-length(x)) # Order the RaceDesc data by its total number

data$RaceDescID <- reorder(data$RaceDescID, data$RaceDescID, function(x)-length(x)) # Order the RaceDescID data by its total number

data$RecruitmentSource <- reorder(data$RecruitmentSource, data$RecruitmentSource, function(x)-length(x)) # Order the RecruitmentSource data by its total number
```
...
data %>% 
  ...
  facet_wrap(~ Department) +
  guides(fill = "none") + 
  labs(title = 'Recruitment Source accross Department') +
  theme(plot.title = element_text(hjust = 0.5),
        # panel.grid.major = element_blank(), 
			  # panel.grid.minor = element_blank(),
        panel.border = element_blank(),
...
p = data_ %>% 
  ...
  labs(fill = "Recruitment Source", title = 'Year: {closest_state}')
